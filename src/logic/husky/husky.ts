...
class Husky implements BaseLogic {
	private static async installHusky(packageManager: PackageManager): Promise<void> {
		try {
			...
			if (shouldAutoInstall) {
				...
				Logger.info(`For manual installation just type: ${huskyInstallCommand}`);
			}
		} catch (error) {
	...
	private static async installLintStaged(packageManager: PackageManager): Promise<void> {
		try {
			const lintStagedInstallCommand = getInstallCommand(packageManager, 'lint-staged', true);
			const shouldAutoInstall = await getConfirmation(
	...
	private static createLintStagedConfiguration(): void {
		...
		const preCommitHuskyLocation = path.join(process.cwd(), '.husky', 'pre-commit');
		const preCommitHusky = fs.readFileSync(preCommitHuskyLocation).toString();
		const modifiedPreCommit = preCommitHusky.replace(/npm.*/, 'npx lint-staged');
