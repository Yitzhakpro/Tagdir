...
export const getPackageJsonPath = (basePath?: string): string => {
	const packageJsonLocation = path.join(basePath ?? process.cwd(), 'package.json');
...
export const isPackageInstalled = (packageName: string): boolean => {
	...
	if (packageJson.devDependencies?.[packageName] || packageJson.dependencies?.[packageName]) {
		return true;
	}
...
export const addScriptToPackageJson = (name: string, script: string): void => {
	...
	fs.writeFileSync(packageJsonLocation, JSON.stringify(packageJson, undefined, 2));
};
...
export const getInstallAllDepsCommand = (packageManager: PackageManager): string => {
	if (packageManager === 'npm') {
		return 'npm i';
	} else if (packageManager === 'yarn1' || packageManager === 'yarn2') {
		return 'yarn';
